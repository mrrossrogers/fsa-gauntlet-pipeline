...
export async function callClaude(system, userContent, { maxTokens = 4096 } = {}) {
  ...
  // stop_reason: "max_tokens" means the response was genuinely cut off by
  // the budget above, as distinct from a parse failure on a complete
  // response -- surface it explicitly so a truncation is never confused
  // with some other kind of malformed output.
  if (data.stop_reason === 'max_tokens') {
    throw new Error(`Claude response was cut off by max_tokens (${maxTokens}). Raise maxTokens for this call.`);
  }

  const text = data.content?.map(b => b.text || '').join('\n') ?? '';
  const clean = text.replace(/```json|```/g, '').trim();
  if (!clean) {
    throw new Error(`Claude returned no parseable content. stop_reason: ${data.stop_reason}. Raw response: ${JSON.stringify(data)}`);
  }

  try {
    return JSON.parse(clean);
  } catch {
    // Full text here, not truncated -- a truncated error message is exactly
    // what makes a truncation bug hard to diagnose from the outside.
    throw new Error(`Claude response wasn't valid JSON (stop_reason: ${data.stop_reason}, length: ${clean.length}): ${clean}`);
  }
}
