...
block only for a material food-safety risk. If secondary_category is supplied,
the piece was deliberately allowed to overlap that other room; do not flag
material belonging to that secondary room as off-desk drift, since this desk's
standard still governs the piece overall as its primary room.
`,
  sex: `
...
or sexual-health risk. If secondary_category is supplied, the piece was
deliberately allowed to overlap that other room; do not flag material
belonging to that secondary room as off-desk drift, since this desk's standard
still governs the piece overall as its primary room.
`,
  alcohol: `
...
material safety or health risk, or language that pressures consumption. If
secondary_category is supplied, the piece was deliberately allowed to overlap
that other room; do not flag material belonging to that secondary room as
off-desk drift, since this desk's standard still governs the piece overall as
its primary room.
`,
};
...
not make publication wait for bespoke photography. Never invent an asset URL,
credit, or license.
...
volume or impairment. Supply search terms, likely source types, and what to
avoid. The article must arrive at owner review with a hero image already
attached.

Write hero_caption and hero_alt_text as the actual, finished, reader-facing
text that will run under and describe the hero image -- not direction or
guidance about how someone else should write them, and never a description of
your own reasoning or process. hero_caption should be one short, functional
sentence tied to what is actually in frame (an ingredient, a ratio, a gesture,
an object), never mood poetry, and never editorializing about solitude,
secrecy, or emotional weight. hero_alt_text should be one literal, accessible
sentence describing what a screen reader user needs to know is visible.
Neither field should ever mention that the image was generated, a model, a
prompt, or anything about how the piece or its art was produced -- both are
reader-facing captions, not production notes.
...
Also write reader_verdict: one to two sentences, in FSA's own voice, that
directly answer "was this experience worth the reader's limited time on
Earth?" This is the only part of your response a reader ever sees on the
article itself; write it as the magazine speaking to them, not as a note to
the owner. Open with a plain verdict (yes, only if, skip, worth it for the
right person, and close variants), then justify it in one more sentence at
most. Never mention desks, critiques, sourcing gaps, blocking issues, image
rights, or anything about how the piece was produced or reviewed; that
belongs in reasoning above, for the owner only, never in reader_verdict.
`;
...
Each candidate may optionally carry a secondary_category from one of the other
two rooms when the idea genuinely and honestly overlaps both, as long as one
room stays clearly primary: a Food candidate whose scene turns on a shared
bottle of wine, or a Sex candidate that opens over a cooked meal, can carry
that overlap as secondary_category rather than being forced into a single lane
that flattens it. Food ideas are harder to source than Sex or Alcohol in this
system; when a pure-food angle is running thin, look first for an honest
Food+Alcohol or Food+Sex overlap before resorting to a generic or repetitive
food idea. Set secondary_category to null when a candidate is genuinely
single-room. Do not manufacture overlap that is not really there just to hit a
quota.
`;
...
export const AGENT_SCHEMAS = {
  ...
  recommendation: {
    type: "object",
    additionalProperties: false,
    required: ["decision", "fsa_verdict", "reader_verdict", "reasoning", "for_whom", "skip_if", "what_it_makes_possible", "verification_type", "value_scorecard", "final_checks"],
    properties: {
    ...
    properties: {
      candidates: {
        ...
        items: {
          type: "object",
          additionalProperties: false,
          required: ["category", "secondary_category", "seed", "angle", "reader_question", "reader_promise", "reporting_path", "originality_risk", "visual_opportunity"],
          properties: {
            category: { type: "string", enum: ["food", "sex", "alcohol"] },
            secondary_category: { type: ["string", "null"], enum: ["food", "sex", "alcohol", null] },
            seed: { type: "string" },
            angle: { type: "string" },
            reader_question: { type: "string" },
            reader_promise: { type: "string" },
