...
export default async function handler(req, res) {
  ...
  try {
    const { data, error } = await getSupabase()
      .from("fsa_articles")
      .select("id,category,secondary_category,format_lane,seed,angle,status,issue,brief,final_decision,final_notes,draft_round,image_round,image_url,image_brief,draft_meta,site_url,approvals,created_at,updated_at")
